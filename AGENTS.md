...
- Routes:
  - `/` redirects to the default profile homepage `/ai-ml`.
  ...
  - `/project/:id` is a legacy redirect to `/ai-ml/project/:id` when that project exists.
...
`npm run lint` is TypeScript-only today. `npm run test` validates portfolio content and layout contracts, including Sai identity proof, Northeastern/KIIT education proof, profile-specific resume links, and compact footer rules. There is no automated browser test suite in this repo yet.
