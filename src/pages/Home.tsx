...
function measureSceneSectionRange(
  ...
  const sceneTarget =
    sectionName === "education"
      ? container.querySelector<HTMLElement>('[data-home-scene-anchor="northeastern-education"]') ?? section
      : sectionName === "contact"
...
function getHeroProofItems(portfolioData: PortfolioData) {
  const currentCompany = portfolioData.experience[0]?.company ?? "Crewasis.ai";
  const hasNortheastern = portfolioData.education.some((entry) => entry.school.includes("Northeastern"));
  const hasKiit = portfolioData.education.some((entry) => entry.school.includes("Kalinga"));

  return [
    { label: "Current", value: currentCompany },
    { label: "Focus", value: portfolioData.personal.focusAreas[0] ?? "ML Systems" },
    { label: "Graduate", value: hasNortheastern ? "Northeastern MS" : "Data Science MS" },
    { label: "CS", value: hasKiit ? "KIIT CSE" : "Computer Science" },
    { label: "Base", value: portfolioData.personal.location },
  ];
}
