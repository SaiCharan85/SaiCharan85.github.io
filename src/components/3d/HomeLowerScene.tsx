...
// --- ACADEMIC EDUCATION SCENE ---

function AcademicFoundationCore({
  simplified,
  ...
  return (
    <group ref={coreRef}>
      ...
      <BrandPlate
        ...
        sublabel="DATA SCIENCE + CS FOUNDATION"
...
function EducationFoundationScene({
  ranges,
  profile,
}: {
  ranges: HomeSceneRanges;
  ...
  useFrame((state) => {
    ...
    const arrivalPulse = getChapterArrivalPulse(progress);
    const educationCardExit = 1 - THREE.MathUtils.smoothstep(progress, 0.76, 0.96);
    const calm = THREE.MathUtils.smoothstep(
      scroll.offset,
      Math.max(0, ranges.contact.start - 0.08),
      Math.min(1, ranges.contact.start + 0.02)
    );
    const alpha = clamp01(presence * (0.96 + arrivalPulse * 0.22)) * educationCardExit * (1 - calm * 0.72);
...
export function HomeLowerScene({
  ...
  return (
    <>
      ...
      <EducationFoundationScene ranges={sectionRanges} profile={profile} />
