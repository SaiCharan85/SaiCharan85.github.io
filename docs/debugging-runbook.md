...
3. Direct hash entry such as `/ai-ml#projects` or `/ai-ml#education` still skips the hero reset and scrolls to the target section.
...
2. The built `dist/` directory contains:
   - `ai-ml/index.html`
   - `datascientist/index.html`
   - at least one profile project path such as `ai-ml/project/innovateai-rag-literature-review-engine/index.html`
   - at least one legacy path such as `project/enterprise-data-lake-modernization/index.html`
